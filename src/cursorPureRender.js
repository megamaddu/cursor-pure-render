...
function shouldComponentUpdate (nextProps, nextState) {
  ...
  const filteredCurrentProps = filter(this.props, isNotIgnorable)
...
function isEqualProps (value, other) {
  ...
  return isEqual(value, other, (current, next) => {
    ...
    const cursorsEqual = compare(current, next, isCursor, isEqualCursor)
    if (cursorsEqual !== void 0) return cursorsEqual
...
export function enableRenderDebug (pattern, logFn) {
  ...
  debug = function (str) {
    ...
    const key = element && element.key ? ' key=' + element.key : ''
